  ...
  const handleCreateUser = async () => {
    try {
      ...
      for (const endpoint of endpoints) {
        try {
          ...
          if (response.ok) {
            ...
            addDebugInfo(`📥 Response: ${JSON.stringify(responseData)}`);
            break;
          } else {
            lastError = responseData.error || 'Unbekannter Fehler';
            addDebugInfo(`❌ Fehler ${response.status} bei ${endpoint}: ${lastError}`);
          }
        } catch (e) {
          lastError = e.message;
          addDebugInfo(`❌ Netzwerkfehler bei ${endpoint}: ${e.message}`);
      ...
      if (success) {
        ...
        addDebugInfo(`❌ Benutzer-Erstellung fehlgeschlagen: ${lastError}`);
        alert(`Fehler beim Erstellen des Benutzers: ${lastError}`);
      }
    } catch (error) {
  ...
  const handleUpdateSubscription = async (userId, newSubscription) => {
    try {
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (responseData.user && responseData.user.subscription === newSubscription) {
              ...
              addDebugInfo(`🎯 Bestätigt: ${userToUpdate.username} hat jetzt ${newSubscription} Subscription`);
              break;
            } else {
              lastError = `Subscription nicht aktualisiert. Erwartet: ${newSubscription}, Erhalten: ${responseData.user?.subscription}`;
              addDebugInfo(`⚠️ ${lastError}`);
            }
          } else {
            lastError = responseData.error || 'Unbekannter Fehler';
            addDebugInfo(`❌ Fehler ${response.status} bei ${endpoint}: ${lastError}`);
      ...
      if (success) {
        ...
        alert(`✅ Subscription erfolgreich zu ${newSubscription} geändert!`);
      } else {
        addDebugInfo(`❌ Subscription-Update fehlgeschlagen: ${lastError}`);
        alert(`Fehler beim Aktualisieren der Subscription: ${lastError}`);
      }
    } catch (error) {
  ...
  const handleDeleteUser = async (userId) => {
    try {
      ...
      if (success) {
        ...
        addDebugInfo(`❌ Benutzer-Löschung fehlgeschlagen: ${lastError}`);
        alert(`Fehler beim Löschen des Benutzers: ${lastError}`);
      }
    } catch (error) {
